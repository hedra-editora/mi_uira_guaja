...
\begin{center}
\textsc{f} = \emph{tu} \emph{→} \textsc{fb} = \emph{tu-nã}\medskip

\textsc{m} = \emph{ihí →} \textsc{mz} = \emph{ihi-nã}
\end{center}

Assim, ambos os sexos reconhecem o pai (\textsc{f}) como \emph{tu} e o irmão do
pai (\textsc{fb}) como \emph{tu"-nã}; e uma vez, ao lhes perguntar sobre tal
...
colaterais (\textsc{fb} e \textsc{mz}) ganham o tecnônimo -\emph{kî} marcando essa
diferença, assim \textsc{f} = \emph{yimî} e \textsc{fb} = \emph{yimkî}. Trata"-se aqui ``do
...
uma mulher, seria sua irmã (\textsc{z}) e suas primas paralelas bilaterais (\textsc{mzd}/
\textsc{fbd}). Por isso, o termo utilizado por ego masculino para \textsc{b}/\textsc{fbs} e por ego
feminino para \textsc{z}/\textsc{mzd}/\textsc{fbd} é \emph{harapihiara}, obedecendo"-se a
...
entre si até a época do contato entre a \textsc{funai} e o grupo de Panaxĩa, em
...
principalmente nos assuntos referentes à relação com a \textsc{funai}, o trabalho
...
viver em sua aldeia original, sem nunca mais voltar à Juriti.\footnote{Dada
  ...
  Terras Indígenas por transportes oficiais (\textsc{funai} ou Funasa), dormir em
...
(Tiracambu, Awá) e, sem mencionar os moradores da aldeia do \textsc{pin}
...
\textsc{funai} resolveu abrir uma trilha por dentro da \textsc{ti} Caru que ligasse a
aldeia do \textsc{pin} Awá à do \textsc{pin} Juriti,\footnote{Nesta época, os Guajá estavam
  sob a administração regional da \textsc{funai} de Belém, que tomava decisões
...
dos funcionários da \textsc{funai}, estavam muitos jovens da aldeia Juriti, além
...
(\emph{pyhy}, ``tomar'') em casamento por Takamỹ, da aldeia do \textsc{pin} Awá.
...
roças; como eram as estruturas da \textsc{funai}; se havia mulheres; como eram
...
nimi'ũa}, ``comida de índio brabo'', se refere à subdieta pela qual
...
\section{\emph{Amõ Awa}, os outros}
...
\emph{Tirakamupahara} (as pessoas que vivem no \textsc{pin} Tiracambu); \emph{awa
Awapahara} (aqueles que vivem na aldeia do \textsc{pin} Awá), \emph{a'e rawỹ awa}
...
grupos vivendo em isolamento voluntário, tanto entre as \textsc{ti}s Awá e Caru
(onde realizo pesquisa) quanto na \textsc{ti} Arariboia (onde se encontram grupos
...
área indígena Arariboia é superestimado em até 60 pessoas (ver \textsc{funai}
...
contato oficial com a \textsc{funai}, os chamados ``isolados''. Tais coletivos
...
pela \textsc{funai}, e sim pelos próprios Guajá da \textsc{ti} Caru, quando trouxeram para
...
buscá"-los. Além de Tatuxa'a, manifestou interesse em ir junto um grupo
...
(por causa da onipresença da \textsc{funai}) e desprezo (pela forma que um
\emph{awatea}, independentemente da \textsc{funai}, o enxergará), pois seriam
...
\section{A família de Takwarẽxa'a}
...
novamente e vive na aldeia do \textsc{pin} Awá. Sobre a família de Takwarẽxa'a, a
...
dividida pela sede do \textsc{pin}. As poucas vezes em que Takwarẽxa'a visita a
aldeia é para buscar provisões de carnes desprezadas pelos Guajá ou para
acompanhar os outros nos trabalhos de roça, coordenados pela \textsc{funai}. Suas
...
estabeleciam como resultado do contato com a \textsc{funai} (Cormier, 2003, p.
89).
...
A relação entre as pessoas da aldeia e a família de Takwarẽxa'a é
...
O filho mais velho de Takwarẽxa'a, Ki'ipi, hoje (em 2015) com 23 anos,
...
nos postos da \textsc{funai}, vários dos animais consumidos por Takwarẽxa'a (como
...
irmãos para ``viverem juntos'' (\emph{ikwẽ pyry}) na aldeia do \textsc{pin} Juriti,
...
relação de afinidade (como \textsc{wb}, por exemplo), até \emph{ka'á rapihiara}
...
postos da \textsc{funai}, os temas que se repetiam em todas as conversas eram
...
metade do século \textsc{xix} (ver Balée, 1994). Apesar da escassez de fontes,
...
coco'' (Ribeiro, \emph{op. cit.}, p. 373). Durante todo o século \textsc{xx}, nas bacias
...
da Terra Indígena Awá, onde se baseia o \textsc{pin} Juriti, só encontram os
...
administração da \textsc{funai}. Sendo assim, os Guajá de cada uma dessas aldeias
...
quanto na \textsc{ti} Awá), a relação entre Guajá e \emph{Tenetehara} é, pela
...
campo, na mesma época em que a \textsc{funai} do Maranhão estava passando por uma
reestruturação, devido a recente mudança na presidência da \textsc{funai} em
Brasília.\footnote{Havia saído Mércio Gomes e entrado Márcio Meira.} No
meio dessa mudança, os \emph{Tenetehara} da \textsc{ti} Caru prenderam em sua
aldeia (chamada Maçaranduba) alguns funcionários da \textsc{funai} e \textsc{funasa},
enquanto outros foram fazer reivindicações no escritório da \textsc{funai} em
...
Caru.\footnote{Tal como colocado por Ribeiro (op cit: 332), os Guajajara
  ...
  reivindicações à \textsc{funai}. Isso não representa que os \emph{Tenetehara}
...
\textsc{funai} no município de Santa Inês para informar sobre minha ida, já que
era a primeira vez em que eu me deslocaria até lá. Alguns Guajajara que
estavam na \textsc{funai} me disseram para não ir para aquela aldeia naquele
...
\emph{Tenetehara}, Guajá e \textsc{funai} na região do Pindaré, creditaram aos
...
funcionários da \textsc{funai} na aldeia Maçaranduba foi Dalmo Guajajara (nome
...
\emph{Tenetehara} não assumindo a chefia do \textsc{pin} Awá, alguns homens Guajá
...
fora dado (com ajuda de alguns funcionários da \textsc{funai}) quando foi viver
...
pessoas da aldeia Juriti, o nome ``Kamara'' lhe foi aplicado pela ``\textsc{funai}'',
...
consagrados pela \textsc{funai} --- uma vez que, como veremos, os nomes para os
...
fornecido informação alguma sobre o destino \emph{post"-mortem} dos
...
Guajá, da aldeia do \textsc{pin} Awá, com mulheres \emph{Tenetehara}
...
Três irmãos entre si que foram criados na aldeia do Awá, Pakẽa, Itaxĩa e
...
a Vale e a \textsc{funai}. Além desses, outro Guajá chamado Iwarata'ĩa vivia no
...
política de assistência da \textsc{funai} que, segundo se queixam, é carente de
...
Algo comum de ouvir entre os funcionários da \textsc{funai} e \textsc{sesai} na região é
...
\emph{Tenetehara} continuarem a vender a madeira da \textsc{ti} Caru e a criarem
...
\emph{Tenetehara} ambíguo, pois, antes de sua demissão da \textsc{funai} e
...
\emph{Tenetehara} nos bloqueios da \textsc{e. f.} Carajás, na altura de Alto Alegre
do Pindaré, para protestar contra a própria \textsc{funai} (eventos que ganhavam
...
nova presidência da \textsc{funai} substituiu os administradores do Maranhão e os
